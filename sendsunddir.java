...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int REQUEST_OPEN_FILE_CODE = 300;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Check if the user wants to record or select a file
                if (outputFile.exists()) {
                    // Encrypt and send the recorded audio
                    encryptAudio();
                    sendAudio();
                } else {
                    // Open a file chooser to select a sound file
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_OPEN_FILE_CODE && resultCode == RESULT_OK) {
            ...
            try {
                ...
                // Encrypt and send the audio file
                encryptAudio();
                sendAudio();
            } catch (Exception e) {
    ...
    private void recordAudio() {
        ...
        if (frequency <44100 || frequency >48000) {
            Toast.makeText(this, "Frequency must be between44100 and48000 Hz", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void sendAudio() {
        ...
        AudioTrack audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC,44100, AudioFormat.CHANNEL_OUT_MONO, AudioFormat.ENCODING_PCM_16BIT, AudioRecord.getMinBufferSize(44100, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT), AudioTrack.MODE_STREAM);
        ...
        try {
            ...
            byte[] buffer = new byte[AudioRecord.getMinBufferSize(44100, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT)];
            int bytesRead;
            while ((bytesRead = fis.read(buffer)) != -1) {
    ...
    private void receiveAudio() {
        ...
        try {
            AudioRecord audioRecord = new AudioRecord(MediaRecorder.AudioSource.DEFAULT,44100, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT, AudioRecord.getMinBufferSize(44100, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT));
            audioRecord.startRecording();
            byte[] buffer = new byte[AudioRecord.getMinBufferSize(44100, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT)];
            int bytesRead;
            while ((bytesRead = audioRecord.read(buffer,0, AudioRecord.getMinBufferSize(44100, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT))) != -1) {
                audioTrack.write(buffer,0, bytesRead);
            }
            audioRecord.stop();
            audioRecord.release();
